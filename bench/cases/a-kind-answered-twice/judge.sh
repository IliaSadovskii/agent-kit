...
grep -q 'тут нет типов' "$STEP/attempt-1/raw.txt" ||
  { echo "the trap was not planted: the record carries no second answer"; exit 1; }

grep -q 'kind-excused-and-commanded: types' "$STEP/attempt-1/refusal.txt" ||
  { echo "the record was not refused: $(cat "$STEP/attempt-1/refusal.txt" 2>/dev/null)"; exit 1; }
exit 0
