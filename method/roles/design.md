...
2. **Decide what changes.** One line per place, naming the file and what happens to it.
   Small and obvious beats clever: the next session has to build exactly this.
3. **Name the seams.** Where does this meet code that already exists, and what must not
   break at that join? A seam nobody named is where the next session breaks something.
4. **Decide what will prove it — now, before any code exists.** This is the field that
...
5. **Say what you assumed.** Anything you took as true without checking. For each one,
