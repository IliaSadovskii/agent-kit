...
# Each entry is `  <path>: <release sha> <on-disk sha>`.
#
# Two checksums, because one cannot tell the three cases apart:
#   * release sha — what the kit shipped last time. On-disk content equal to it means the file is
#     untouched and safe to replace.
#   * on-disk sha — what the project actually had after the last run. Equal to it (and with the
#     release file unchanged) means an accepted local customization: keep it, silently.
# Anything else is a genuine conflict: both sides moved.
lock_sha() {
  local rel="$1" field="$2" lock; lock="$(LOCK)"
  [ -f "$lock" ] || return 0
  awk -v want="  $rel: " -v field="$field" \
    'index($0, want) == 1 { split(substr($0, length(want) + 1), parts, " ");
                            print parts[field]; exit }' "$lock"
}
...
# Entries are collected by apply_payload as "<path> <release sha> <on-disk sha>".
write_lock() {
  local version="$1"; shift
  local lock; lock="$(LOCK)"
  {
    printf '# Generated by the agent-kit installer. Do not edit by hand.\n'
    printf '# Each file records two checksums: what the release shipped, and what this project had\n'
    printf '# after the last run. `install.sh update` compares both, so an untouched file is\n'
    printf '# refreshed, a file you customized is kept, and a file that moved on both sides is\n'
    printf '# reported as a conflict instead of being overwritten.\n'
    ...
    for entry in "$@"; do
      read -r path release_sha disk_sha <<<"$entry"
      printf '  %s: %s %s\n' "$path" "$release_sha" "$disk_sha"
    done
  } > "$lock"
}
...
apply_payload() {
  local installed=() rel src dest src_sha disk_sha was_release was_disk
  local n_new=0 n_upd=0 n_same=0 n_kept=0
  ...
  while read -r rel; do
    ...
    if [ ! -f "$dest" ]; then
      ...
      say "  create   $rel"
      n_new=$((n_new + 1))
      LOCK_ENTRIES+=("$rel $src_sha $src_sha")
      continue
    fi
    ...
    disk_sha="$(sha_of "$dest")"
    was_release="$(lock_sha "$rel" 1)"
    was_disk="$(lock_sha "$rel" 2)"

    if { [ -n "$was_release" ] && [ "$disk_sha" = "$was_release" ]; } || [ "$FORCE" -eq 1 ]; then
    ...
    elif [ -n "$was_disk" ] && [ "$disk_sha" = "$was_disk" ] && [ "$src_sha" = "$was_release" ]; then
      # A local customization the project already lives with, and the release did not touch this
      # file. Nothing to decide — keep it quietly instead of re-reporting the same conflict.
      n_kept=$((n_kept + 1))
      LOCK_ENTRIES+=("$rel $src_sha $disk_sha")

    else
      # Both sides moved, or the file predates the lock: keep the local file, park the new one.
  ...
  while read -r known; do
    ...
    [ -f "$dest" ] || continue
    disk_sha="$(sha_of "$dest")"
    was_release="$(lock_sha "$known" 1)"
    if [ "$disk_sha" = "$was_release" ] || [ "$FORCE" -eq 1 ]; then
  ...
  info ""
  info "Payload: $n_new created, $n_upd updated, $n_same already current, $n_kept kept as local"
  info "         customizations, ${#CONFLICTS[@]} conflicts."
}
...
do_install() {
  ...
  if [ "$DRY_RUN" -eq 0 ]; then
    write_lock "$version" "${LOCK_ENTRIES[@]}"
...
# Every kit file whose content differs from what the installed release shipped.
modified_files() {
  local rel was_release current
  while read -r rel; do
    [ -n "$rel" ] || continue
    if [ ! -f "$PROJECT_DIR/$rel" ]; then printf 'missing %s\n' "$rel"; continue; fi
    was_release="$(lock_sha "$rel" 1)"
    current="$(sha_of "$PROJECT_DIR/$rel")"
    if [ "$was_release" != "$current" ]; then printf 'modified %s\n' "$rel"; fi
  done < <(lock_files)
}
