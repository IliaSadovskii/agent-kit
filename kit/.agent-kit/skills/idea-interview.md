...
8. **Open a bootstrap PR and stop.** Commit docs, scaffolding, and manifest on a
   `bootstrap-<slug>` branch, open the PR, and tell the author to merge it before running `/ship`.
...
- **`.agent-kit/project/instructions.md`** — stack, run and cloud notes, the concrete test / lint /
  migration commands, architectural constraints, and the coding-standards path. User-owned,
  survives kit updates. Stack specifics belong here, not in forked copies of the canonical roles —
  `tester` and `reviewer` derive their commands from this file and the manifest.
...
- **`.github/pull_request_template.md`** — propose the sections from
  `.agent-kit/rules/pull-requests.md` and confirm before writing.
...
- **Root `CLAUDE.md`** — preserve the managed block; add only Claude-specific overrides below it.
- Anything else (`.env.example`, linters, a CI stub) only when the stack clearly calls for it.

Don't generate a per-project "how this kit works" document — that is `.agent-kit/GUIDE.md`, which
