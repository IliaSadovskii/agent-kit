---
name: infra-cloud
description: Take the project from runs-locally to reachable-on-the-internet — pick a host, generate deploy config and CI, guide owner-only provisioning, and record the runbook.
---
...
`${CLAUDE_PLUGIN_ROOT}/skills/infra-local/references/mobile-env.md`. Hosting decision tree and provider trade-offs:
`${CLAUDE_PLUGIN_ROOT}/skills/infra-cloud/references/hosting-catalog.md`.
...
8. **Write the runbook section** — create/update `docs/deployment.md` (path from
   ...
   **"Manual actions"** subsection (owner-only steps, mirroring the format in
   `${CLAUDE_PLUGIN_ROOT}/rules/pull-requests.md`).
