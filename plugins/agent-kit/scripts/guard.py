...
for segment in re.split(r"\|\||&&|;|\|", command):
    ...
    if words[:3] == ["gh", "pr", "merge"]:
        ask("agent-kit: the kit never merges pull requests — the owner merges. "
            "Confirm to override.")
    ...
    if any(w == "-f" or w.startswith("--force") for w in rest):
        ask("agent-kit: force push rewrites remote history. Confirm to override.")

    default = git("symbolic-ref", "--short", "refs/remotes/origin/HEAD").rpartition("/")[2] or "main"
    ...
    if refspecs:
        for spec in refspecs:
            ...
            if dest == default or dest.endswith("/" + default):
                ask(f"agent-kit: this push targets the default branch ({default}); "
                    "the kit works on feature branches. Confirm to override.")
    elif git("rev-parse", "--abbrev-ref", "HEAD") == default:
        ask(f"agent-kit: pushing from the default branch ({default}); "
            "the kit works on feature branches. Confirm to override.")
