...
| `run.json` → `manual` | the run that found it — only what needs the owner's hands *and* access, each carrying a `proof` that is a **command** | the closing session, composing **Manual actions** and copying them into `docs/manual.md`; `accept` | the run finishing — the record that outlives it is the file below | as above |
...
| `docs/manual.md` | the closing session, copying every child's `manual` in the same commit as the ledger; a run delivering its own pull request | `check.py` before every command, printing only what this project's `stage` has reached; `next`; `accept` | **the program** — `check.py --manual` runs each action's `proof` and deletes the lines whose work has happened; the owner, for the few that no command can answer | git |
...
| `verification.yml` **in the plugin** | the kit itself — a kind added here is asked of every project on its next check | `check.py`, holding each project's answers against it; `blueprint`, walking it with the owner; `ship` and an epic's proving phase, for which kinds they run | a commit that deletes the entry and the test naming it | git, in the kit |
| `project.yml` → `verification` | `blueprint`, with the owner, judging from the repository and not from `stack.md` — a command that runs the kind, or `no <date> <reason>` | `check.py`, which holds a command to starting and a refusal to a date; `ship`, choosing which kinds this change touches; an epic's proving phase; `--tests` | `blueprint`, retaking the answers — `checks.verification_reviewed` is what says when that is due, at six months or when a dependency manifest moves | git |
| `run.json` → `verified` | the feature, one record per kind it ran — `kind`, `command`, `result`, or `why` it could not apply | `check.py --run`, which refuses a finished feature that is silent about a kind this project answered for; the closing session, into the pull request | the run finishing | the machine only |
...
| `.github/workflows/<name>.yml` | a `ship` with a task, from `templates/workflow.yml` — never generated by a command of this kit | GitHub, on every push; `check.py --tests`, which says whether anything outside a session runs each declared command | the owner, on the day the project stops using it; any run that changes what the project declares rewrites it, and `--tests` is what says it has gone stale | git |
