...
| `project.yml` → `commands.visual` / `commands.contract` | `blueprint`, with the owner, judging from the repository and not from `stack.md` | `ship`, which runs `visual` on a feature that changed a screen; an epic's proving phase, which runs `contract` — the one moment anything calls the real service; `check.py`, which holds each to starting; `--tests` | `blueprint`, with the owner, on the day the project stops using it | git |
| `project.yml` → `tests.visual` / `tests.contract`, `checks.sight_reviewed` | `blueprint` — **a refusal only**, `no <date> <reason>`, and only where the command above is empty. A claim goes in the command, where a program can start it | `check.py`, which holds a refusal to a date and a reason and asks again at six months; every build command, through `rules/preflight.md` | `blueprint`, retaking it — or the day a command is written, which replaces it | git |
