...
- **Один идентификатор на два проекта.** `identifier(slug, question)` не знает проекта, а `id` —
  первичный ключ. Два прогона с одинаковым именем и одинаковым вопросом в разных проектах
  делят строку. Редко, лечится составным ключом; не сделано.
- **`until` не обновляется при повторной отправке.** Драйвер, поднявший шаг заново, шлёт вопрос
  новым сообщением, а реестр помнит старое: реплай после этого не находит вопрос, а выметание
  может снести строку из-под живого драйвера. Не сделано.
- **Падение канала на втором вопросе стирает первый**, уже доставленный. Не сделано.
- **`owner setup` читает `getUpdates` мимо аренды читателя.** Практически безопасно — канала в
  этот момент ещё нет, — но правило про одного читателя нарушено именно там.
