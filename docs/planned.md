...
| **A `/agent-kit:deploy` command** — cold-boot the app, write the CI, write a deploy script, later help with production | Proposed 17 August 2026 and cut the same day, against the files. Three of its five premises were false: the cold boot already exists and runs in every epic (`epic/references/finish.md:99-121`), the project it was proposed for had run its browser suite 42 times in that very phase, and `commands.health` already existed there as a `wait-health` target `commands.run` depends on. Of the rest, the gate condition would have refused an epic on every project while recommending a command that did not exist yet; the `tests:` table was four fields of prose in a config file with no reader and no closer; and the "last executed" column was to be filled from `gh run list`, which answers per workflow run and knows nothing about a kind of test — into a file only `blueprint` may write (`rules/channels.md:45`). What survived is in 2.23.0: one screen, derived from what already exists, and one line where the owner is standing. **Building the CI is an ordinary `ship` with a task**, from `templates/workflow.yml`. |
| **The kit deploying to production** | It can hold the shape — a declared command, a proof it ran, a list for the owner's hands, the `stage` gate — and it refuses the specifics, for one sufficient reason: **nothing in this kit can promise a rollback.** A delivery mechanism without one is worse than a manual step, because it makes the irreversible cheap. Revisit only with the merge policy, which is where a program first gets the right to act on a green result. |
